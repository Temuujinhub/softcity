...
export const metadata: Metadata = { title: "Чадавх бэхжүүлэх сургалт | Зөөлөн хот" };
export const dynamic = "force-dynamic";

export default async function TrainingPage() {
  const content = await getContent();
  const topics = content.training_topics.split("\n").map((s) => s.trim()).filter(Boolean);
          ...
          <div className="max-w-3xl">
            {topics.map((topic, i) => (
              <div key={i} className="flex items-baseline gap-6 py-5 border-b border-[#e7e2d9]">
                ...
                <p className="text-lg sm:text-xl font-medium">{topic}</p>
              </div>
            ))}
          </div>
          <div className="mt-12">
